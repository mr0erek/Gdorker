import requests, random, concurrent.futures, os, base64, json, re, contextlib, argparse, sys, time
...
"Mozilla/5.0 (iPhone; U; fr; CPU iPhone OS 4_2_1 like Mac OS X; fr) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile8C148a Safari/6533.18.5",
"Mozilla/5.0 (iPod; U; CPU iPhone OS 4_2_1 like Mac OS X; he-il) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8C148Safari/6533.18.5", 
"Mozilla/5.0 (iPod; U; CPU iPhone OS 4_3_1 like Mac OS X; zh-cn) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8G4Safari/6533.18.5",
...
"Mozilla/5.0(iPad; U; CPU iPhone OS 3_2 like Mac OS X; en-us) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Mobile/7B314Safari/123",
"Mozilla/5.0(iPad; U; CPU iPhone OS 3_2 like Mac OS X; en-us) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Mobile/7B314Safari/531.21.10",
"Mozilla/5.0(iPad; U; CPU iPhone OS 3_2 like Mac OS X; en-us) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Mobile/7B314Safari/531.21.10gin_lib.cc",
...
] # cat dork1.list | grep -o '".*"'  | sed 's/"/\", \"/g' | sed 's/^",[[:space:]]\(.*\)"$/\1/' >> dork1.list
...
def fetch_url(url):
    ...
    # raw_cookies = "CONSENT=YES+updated_cookie_value"
    # Convert the raw cookies string into a dictionary
    #cookies = requests.utils.cookiejar_from_dict({cookie.split("=")[0]: cookie.split("=")[1] for cookie in raw_cookies.split("; ")})
    cookies = {
    "CONSENT": "YES+srp.gws-20211028-0-RC2.es+FX+330"
    }
    core(url, proxy_list, UA, cookies)

def core(url, UA, proxy_list, cookies):

    LIST_DORKS = [FILETYPE, INURL, INTITLE]
    urls = []
    for search_query in LIST_DORKS:
        ...
        print(f"{GREEN}[{WHITE}+{GREEN}] {YELLOW}Random proxy : {WHITE}{_PROXY}")
        time.sleep(0.5)
        print(f"{GREEN}[{WHITE}+{GREEN}] {YELLOW}Random User-Agent : {WHITE}{_UA}")
        session = requests.Session()
        session.proxies.update(proxies)
        print(f"{GREEN}[{WHITE}+{GREEN}] {YELLOW}G-dorks : {WHITE}")
        for i in search_query:
            print(f"  {i}")
        for dorks in search_query:
            print(f"{GREEN}[{WHITE}+{GREEN}] {YELLOW}checking for  : {WHITE}{dorks}\n")
            search_url = f"http://www.google.com/search?q={url}%20{dorks}&start={start}&client=firefox-b-e"
            ...
            time.sleep(2)
            uri = None
        urls, soup, response, search_url, link, dorks, i, = [None] * 8
...
88eee8      88eee8 8eee8 88   8 88   8 88ee 88   8 {YELLOW}v{VERSION}{WHITE}  
                                                  {YELLOW}by Shiv Patel - (@e343io){WHITE}
{WHITE}---------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    #parser.add_argument("--proxy", help="Proxy address", type=str)
    parser.add_argument("--site", help="Site to search", type=str)
    args = parser.parse_args()
    if args.site:
        banner()
        urls = [f"{args.site}"] 
        #["securemetasys.com", "http://www.google.com", "http://whatsmyip.org", "http://www.facebook.com"] #max_workers = 4 
        for url in urls:
            fetch_url(url)
    else:
        print(f"Please run the tool again as,\n./{sys.argv[0]} --site <site>") #<proxy:port>
